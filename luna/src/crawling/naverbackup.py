...
# print("=== 클래스 패턴 분석 ===")
# # 포함할 클래스 패턴(메인 기사)
# include_pattern = "m6eR7KmFjLYOLhYRyXPh"
# # 제외할 클래스 패턴(밑에 유사한 기사)
# exclude_pattern = "m7VJzZ2bSaunam2mv1Xd"
...
#         extracted_title = article.title or title  # 파싱된 제목이 없으면 원래 제목 사용
        
#         # 기사 정보 저장 (html_content 대신 text_content 사용)
#         news_item = {
#             "id": i,
#             "title": extracted_title,
#             "url": url,
#             "domain": domain,
#             "text_content": text_content,  # HTML 대신 추출된 텍스트 저장
...
# # 새 기사 추가 또는 기존 기사 업데이트
# for article in news_data:
#     # ID 생성 (기존 ID가 있으면 유지)
#     article_id = str(article["id"])
    
#     # 기존 DB에 URL이 같은 기사가 있는지 확인
#     existing_article = next((a for a in db_data["articles"] if a["url"] == article["url"]), None)
    
#     if existing_article:
#         # 기존 기사 업데이트
#         existing_article["title"] = article["title"]
#         existing_article["domain"] = article["domain"]
#         # html_content 대신 text_content 저장
#         existing_article["text_content"] = article.get("text_content", "")
#         existing_article["crawled_at"] = article["crawled_at"]
#     else:
#         # 새 기사 추가
#         db_article = {
#             "id": article_id,
...
#         }
#         db_data["articles"].append(db_article)
...
# # 메타데이터에 저장
# db_data["metadata"]["last_crawled"] = last_crawled.strftime("%Y-%m-%dT%H:%M:%SZ")
# db_data["metadata"]["next_crawl"] = next_crawl.strftime("%Y-%m-%dT%H:%M:%SZ")
