...
export class LocalLLMInterface {
  ...
  private startPythonCrawler(): void {
    try {
      ...
      elizaLogger.info(`Python 크롤러 시작 중... 경로: ${crawlerPath}`);

      const pythonProcess = spawn("/usr/bin/python3", [crawlerPath], {
        detached: true,
        stdio: "ignore",
      });
      ...
      elizaLogger.info(`Python 크롤러 시작됨 (PID: ${pythonProcess.pid})`);
    } catch (error) {
